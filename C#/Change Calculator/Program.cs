    ...
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            do
            {
                ...
                Console.WriteLine("Change Calculator - Jonathan Le");
                Console.WriteLine();
        ...
        static double GetValue(string prompt)
        {
            ...
            do
            {
                ...
                input = input.Replace('$', ' ');
                input = input.Trim();
                //substring starts at position 0, identifies "." and only returns the values after 3 positions from "."
                //only do substring if there is a decimal in the input
                if(input.IndexOf(".") >= 0)
        ...
        static double Normalization(Currency[] array, double currencyAmount)
        {
            ...
            double roundedCurrency = Math.Round((currencyAmount/0.05))*0.05;
            //storing rounded amount to be operated on
            double remainingCurrency = roundedCurrency;
            Console.WriteLine($"User entry of {currencyAmount:C} and rounded to {roundedCurrency:C}");
